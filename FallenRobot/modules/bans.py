...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if not user_id:
        message.reply_text("Saya ragu apakah itu anggota.")
        return log_message
    try:
        ...
        return log_message
    if user_id == bot.id:
        message.reply_text("Oh ya, blokir diriku sendiri, lol!")
        return log_message
    ...
    try:
        ...
        reply = (
            f"<code></code><b>⚠️ BLOKIR</b>\n"
            f"<code></code><code>• Admin:</code> {mention_html(user.id, user.first_name)}\n"
            f"<code></code><code>• Anggota:</code> {mention_html(member.user.id, html.escape(member.user.first_name))}"
        )
        if reason:
            reply += f"\n<code>• Alasan:</code> {html.escape(reason)}"
...
def unban(update: Update, context: CallbackContext) -> str:
    ...
    message.reply_text("Yep, anggota itu sekarang sudah bisa untuk bergabung lagi!")
